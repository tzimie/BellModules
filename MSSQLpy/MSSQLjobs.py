...
q0 = """select job_id,max(convert(varchar,run_date)+convert(varchar,run_time)) as last
...
f0 = """select job_id,max(convert(varchar,run_date)+convert(varchar,run_time)) as last
  into #l from msdb.dbo.sysjobhistory H where step_id=0
  group by job_id"""
f1 = """select J.name as job_name, H.step_name, H.message
...
print('<h1>SQL server jobs status</h1>')
print(Header)
print(makegrid(d1))

print('<br><br><h1>Fails in last executions</h1>')
d2 = MSSQLqueryHB(conn, f1, f0)
print(makegrid(d2))
