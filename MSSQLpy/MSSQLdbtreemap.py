...
d = MSSQLquery(conn, q)
for r in d:
  data.append( [r[1], r[2], r[3], r[4], r[5]] )

df = pd.DataFrame(np.array(data), columns=['schema', 'table', 'index', 'size', 'itype'])
fig = px.treemap(df, path=['schema', 'table', 'index'], values='size', color='itype',
...
print (f"""<html>
<h1>TreeMap of relative table size in a database {dbname}</h1>
