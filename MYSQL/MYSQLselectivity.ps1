...
$rows = ODBCint $conn "select count(*) as cnt from ``$table``"
if ($rows -eq 0) { $rows = 1 }
...
foreach ($s in $d) { 
  $col = $s.column_name
  $distinct = ODBCint $conn "select count(distinct ``$col``) as cnt from ``$table``"
  if ($distinct -eq 0) { $distinct = 1 }
  $topone   = ODBCint $conn "select count(*) as cnt from ``$table`` group by ``$col`` order by 1 desc limit 1"
